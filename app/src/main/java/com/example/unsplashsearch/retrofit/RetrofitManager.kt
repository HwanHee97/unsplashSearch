...
class RetrofitManager {
    ...
    //사진검색 api 호출
    fun searchPhotos(searchTerm: String?, completion: (RESPONSE_STATE,ArrayList<Photo>?) -> Unit) {//searchTerm글자,
                ...
                when(response.code()){//응답 코드기 200(정상)일떄만 completion을(메인 액티비티로) 보낸다. 정상이 아닌 반환코드가 통신 성공으로 뜰수 있음을 방지
                    200->{
                        response.body()?.let { //response.body()에 데이터가 있다면
                            ...
                            val results=  body.getAsJsonArray("results")//body에서 results란 이름을getAsJsonArray 형태로 가져온다
                            ...
                            results.forEach{resultItem-> //results의 값을 it으로 해도되는데 이와같이 resultItem으로 이름을 정할수 있다. resultItem 은 하나의 사진에 대한 정보
                                ...
                                val resultItemObject=resultItem.asJsonObject //resultItem(하나의 사진)을 jsonobject로 뺴온다.
                            ...
                            completion(RESPONSE_STATE.OKAY,parsedPhotoDataArray)//mainActivity로 Photo객체를 담은 Array리스트를 보낸다.
                        }
                    }
                }
            }
